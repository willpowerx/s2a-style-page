...
- Root cause: every changed rung in the `0.0.21` `--s2a-section-spacing-*` output carried an inline `/** Milo-specific declarations */` comment — component-specific values needed by one Milo component were written directly into the shared scale, corrupting it for every other consumer.
- Reverted `-xl` / `-2xl` / `-3xl` / `-4xl` to distinct, ascending desktop values (the `0.0.20` baseline: 80/96/128/160), restoring full monotonicity at all three breakpoints.
- Also fixed a pre-existing issue while in here: Mobile `-md` (16px) sat below Mobile `-sm` (24px) even in `0.0.20`. Bumped to 24px. This is the one value here that isn't a straight revert — flagged in the proposal doc for explicit sign-off, since a live consumer could depend on the old 16px.
- Added new standalone tokens `--s2a-section-spacing-pivot-{sm,md,lg}` carrying the three values that were removed from the base scale (24/80/128, 40/64/128, 64/80/128 across Mobile/Tablet/Desktop) — these aren't a scale, just three independent tokens for components that converge on a shared desktop ceiling via different ramps.
- Added a "Section Spacing Pivot" reference grid to `index.html`, replaced the ⚠ regression callout with a "locally patched, pending upstream" note, and updated the Version History table.
- **This patch is not yet reflected upstream.** Once the Consonant team ships a real fix (see open questions in the proposal doc), re-sync from the published package and remove the local patch note.
...
- **⚠ Known upstream regression (superseded by the 2026-08-17 local patch above):** `--s2a-section-spacing-*` values changed in this release and are no longer monotonic — Mobile `-2xs` (8px) < `-3xs` (16px) and `-2xl` (24px) < `-xl` (40px); Tablet `-md` (24px) < `-sm` (32px); Desktop/HD collapse `-xl` through `-4xl` to a single 128px value. The upstream `CHANGELOG.md` for this release is self-flagged "auto-generated by the token release pipeline — verify before merge" and doesn't document this drift. Needs a fix or revert from the Consonant token team — do not treat these values as a design decision.
